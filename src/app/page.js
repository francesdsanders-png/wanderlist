...
‘Experience Type’: [‘🍽️ Restaurant’, ‘🍸 Bar’, ‘🌲 Nature’, ‘🛍️ Shopping’, ‘🎭 Entertainment’, ‘🏃 Activity’, ‘🏨 Hotel’, ‘👗 Thrift Shop’],
‘Reservation’:     [‘📋 Reservation Needed’, ‘🚶 Walk-in Welcome’],
‘Hours’:           [‘🌙 Open Late’, ‘☀️ Morning’, ‘🕛 Year-round’, ‘🗓️ Seasonal’],
}
const EMPTY_FORM = {
...
const filteredAll = places.filter(p => {
if (filterStatus === ‘All’) return true
if (filterStatus === “We’re In”) return p.partner_status === “We’re In” && p.trip_status !== ‘Completed’
return p.partner_status === filterStatus
})
...
function PlaceCard({ place, onYes, onPass, onEdit, onDelete, onClick, showTripStatus }) {
...
<StatusChip status={showTripStatus && place.trip_status ? place.trip_status : place.partner_status} />
</div>
...
{ label: “WE’RE IN”, value: approved.length, color: T.green },
...
<PlaceCard key={place.id} place={place}
showTripStatus={true}
onClick={() => { setSelectedPlace(place); setView(‘yesdetail’) }} />
))}
</div>
</>
)
...
onChange={v => {
updatePlace(cur.id, { trip_status: v })
if (v === ‘Completed’) { setSelectedPlace({ …cur, trip_status: ‘Completed’ }); setView(‘compdetail’) }
}} />
<button onClick={() => deletePlace(cur.id)} style={btnDanger}>Remove Place</button>
</div>
</>
...
{completed.map(place => (
<PlaceCard key={place.id} place={place}
onClick={() => { setSelectedPlace(place); setView(‘compdetail’) }} />
))}
</div>
)
}
