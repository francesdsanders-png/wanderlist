    ...
    <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {TRIP_STATUSES.map(s => {
          ...
          return <button key={s} onClick={() => onUpdate(active ? null : s)} style={{ flex: 1, padding: "7px 0", borderRadius: 8, border: `1.5px solid ${active ? sc.border : C.border}`, background: active ? sc.bg : C.bg, color: active ? sc.color : C.textMuted, fontSize: 11, fontFamily: font, fontWeight: active ? 700 : 500, cursor: "pointer" }}>{s}</button>;
        })}
      </div>
    </div>
  );
...
export default function App() {
  ...
  async function addPlace(place) {
    const { error } = await supabase.from('places').insert([{ name: place.name, location: place.location, distance: place.distance, cost: place.cost, website: place.website, phone: place.phone, link: place.link, description: place.description, tags: place.tags, status: 'Undecided', trip_status: null }]);
  ...
  const filtered = places.filter(p => {
    ...
    if (filter === "Completed") return p.trip_status === "Completed";
