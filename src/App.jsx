...
const C = {
  ...
  yellow: "#f59e0b", yellowBg: "#fef3c7", yellowBorder: "#fcd34d",
};
...
function NameField({ value, onChange, nameError }) {
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      {nameError && <div style={{ fontSize: 12, color: C.red, marginTop: 5, fontFamily: font, fontWeight: 600 }}>⚠️ Please enter a place name before saving.</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${C.border}` }}>
      ...
      <div style={{ display: "flex", gap: 6 }}>
        {[1,2,3,4,5].map(star => (
          <button key={star} onClick={() => onRate(rating === star ? 0 : star)} style={{ background: "none", border: "none", fontSize: 26, cursor: "pointer", color: star <= rating ? C.yellow : "#d1d5db", padding: "0 2px" }}>★</button>
        ))}
        {rating > 0 && <span style={{ fontSize: 12, color: C.textSub, fontFamily: font, alignSelf: "center", marginLeft: 4 }}>{rating}/5</span>}
      </div>
    </div>
  );
}
...
function PlaceCard({ place, onUpdate, onDelete }) {
  ...
  const tripStatus = place.tags?.find(t => ["Pending", "Scheduled", "Completed"].includes(t)) || null;
  ...
  function handleTripStatus(val) {
    const filtered = (place.tags || []).filter(t => !["Pending", "Scheduled", "Completed"].includes(t));
    const newTags = val ? [...filtered, val] : filtered;
    onUpdate(place.id, { tags: newTags });
  }

  function handleRating(val) {
    onUpdate(place.id, { rating: val });
  }
  ...
  return (
    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "16px 16px 14px", marginBottom: 10, position: "relative", overflow: "hidden", boxShadow: "0 1px 4px rgba(0,0,0,0.05)" }}>
      ...
      {place.tags?.filter(t => !["Pending","Scheduled","Completed"].includes(t)).length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: 10, marginBottom: 10 }}>
          {place.tags.filter(t => !["Pending","Scheduled","Completed"].includes(t)).map(tag => {
      ...
      {isWeIn && !editing && <StarRating rating={rating} onRate={handleRating} />}
      {!editing && (
