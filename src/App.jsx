...
const FILTER_TABS = ["All", "Undecided", "We're In", "Family", "Adults Only", "Free", "Drive", "Fly", "Morning", "Night", "One-Time", "Regular Spot"];
...
function PlaceForm({ initial, onSave, onCancel, saveLabel = "Save", saving }) {
  ...
  return (
    <div>
      <Field label="Place Name *" value={name} onChange={setName} placeholder="e.g. Crowders Mountain" />
      ...
      <Field label="Notes" value={description} onChange={setDescription} multiline placeholder="Why you saved it…" />
      <TagSection tags={tags} setTags={setTags} />
      <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
        <button disabled={saving} onClick={() => { if (name.trim()) onSave({ name, location, distance, cost, website, phone, link, description, tags }); }} style={{ flex: 1, padding: "13px 0", borderRadius: 12, border: "none", background: saving ? C.blueMid : `linear-gradient(135deg, ${C.blue}, ${C.blueDark})`, color: "#fff", fontSize: 14, fontFamily: font, fontWeight: 700, cursor: saving ? "default" : "pointer" }}>{saving ? "Saving…" : saveLabel}</button>
...
function PlaceCard({ place, onUpdate, onDelete }) {
  ...
  const barColor = place.status === "We're In" ? C.green : place.status === "Pass" ? C.red : C.blue;
      ...
      {!editing && (
        <div style={{ display: "flex", gap: 7, marginTop: 8 }}>
          {[{ label: "✓ Yes", status: "We're In", activeC: C.green, activeBg: C.greenBg, activeBorder: C.greenBorder }, { label: "✗ Pass", status: "Pass", activeC: C.red, activeBg: C.redBg, activeBorder: C.redBorder }].map(({ label, status, activeC, activeBg, activeBorder }) => {
            ...
            return <button key={status} onClick={() => onUpdate(place.id, { status: on ? "Undecided" : status })} style={{ flex: 1, padding: "8px 0", borderRadius: 9, border: `1.5px solid ${on ? activeBorder : C.border}`, background: on ? activeBg : C.bg, color: on ? activeC : C.textMuted, fontSize: 12, fontFamily: font, fontWeight: 600, cursor: "pointer" }}>{label}</button>;
          })}
          <button onClick={() => setEditing(true)} style={{ width: 38, borderRadius: 9, border: `1.5px solid ${C.border}`, background: C.bg, color: C.textMuted, fontSize: 14, cursor: "pointer" }}>✏️</button>
          <button onClick={() => onDelete(place.id)} style={{ width: 38, borderRadius: 9, border: `1.5px solid ${C.border}`, background: C.bg, color: C.textMuted, fontSize: 14, cursor: "pointer" }}>🗑️</button>
        </div>
      )}
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(15,20,40,0.35)", backdropFilter: "blur(6px)", display: "flex", alignItems: "flex-end" }}>
      <div style={{ width: "100%", background: C.surface, borderRadius: "22px 22px 0 0", padding: "24px 20px 44px", borderTop: `1px solid ${C.border}`, maxHeight: "90vh", overflowY: "auto", boxSizing: "border-box", boxShadow: "0 -8px 40px rgba(0,0,0,0.12)" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 999, color: C.textSub, fontSize: 16, cursor: "pointer", width: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
        </div>
...
export default function App() {
  ...
  async function fetchPlaces() {
    ...
    if (error) { setError("Couldn't connect. Check Supabase credentials."); } else { setPlaces(data || []); }
    setLoading(false);
  }

  async function addPlace(place) {
    const { error } = await supabase.from('places').insert([{ name: place.name, location: place.location, distance: place.distance, cost: place.cost, website: place.website, phone: place.phone, link: place.link, description: place.description, tags: place.tags, status: 'Undecided' }]);
      ...
      <div style={{ position: "relative", zIndex: 1, paddingBottom: 100 }}>
        ...
        <div style={{ overflowX: "auto", paddingLeft: 20, paddingBottom: 14 }}>
          <div style={{ display: "flex", gap: 7, minWidth: "max-content", paddingRight: 20 }}>
            {FILTER_TABS.map(tab => { const on = filter === tab; return <button key={tab} onClick={() => setFilter(tab)} style={{ padding: "7px 15px", borderRadius: 999, border: `1.5px solid ${on ? C.blue : C.border}`, background: on ? C.blueLight : C.surface, color: on ? C.blue : C.textSub, fontSize: 12, fontWeight: on ? 700 : 500, cursor: "pointer", fontFamily: font, whiteSpace: "nowrap", boxShadow: on ? `0 0 0 3px ${C.blueMid}55` : "none" }}>{tab}</button>; })}
          </div>
        </div>
        ...
        <div style={{ padding: "0 14px" }}>
          ...
          : loading ? <div style={{ textAlign: "center", padding: "60px 20px", color: C.textMuted, fontFamily: font }}>Loading…</div>
          : filtered.length === 0 ? (
